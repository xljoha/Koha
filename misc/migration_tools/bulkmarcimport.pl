    ...
    unless ($test_parameter) {
        if ($authorities) {
            ...
            if ($matched_record_id) {
                ...
                if ($update) {
                    my $success;
                    eval { $success = ModBiblio($record, $matched_record_id, GetFrameworkCode($matched_record_id), { source => 'bulkmarcimport' }, $modify_biblio_marc_options) };
                    if ($@) {
